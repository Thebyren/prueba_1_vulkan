...
use crate::{DEVICE_EXTENSIONS, PORTABILITY_MACOS_VERSION, VALIDATION_ENABLED, VALIDATION_LAYER};
...
pub unsafe fn pick_physical_device(instance: &Instance, data: &mut AppData) -> Result<()> {
    for physical_device in instance.enumerate_physical_devices()? {
        ...
        if device_features.shader_float64 == vk::TRUE{
            info!("el dispositivo soporta shaderfloat64");
        }else{
            warn!("el dispositivo no soporta shaderfloat64");
        }
    }
